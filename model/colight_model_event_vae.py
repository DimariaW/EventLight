...
class ColightModel(parl.Model):
    def __init__(self, stacked_obs_act_dim, act_dim, edge_index=None, graph_layers=None):
        ...
        #traffic event encoder, event = 0 不发生， event=1 发生
        self.event_transformers = nn.ModuleList([TransformerEncoderLayer(self.obs_dim + self.act_dim) for _ in range(1)])
        self.event_logits_head = nn.Sequential(
            nn.Linear(self.obs_dim + self.act_dim, 2),
        )
    ...
    def attention(self, obs, aux_loss=False, add_noise=False, show_kl=True):
        ...
        if not aux_loss:
            if show_kl:
                ...
                event_hidden = z
                event_hidden = self.pos_encoder(event_hidden)
                for transformer in self.event_transformers:
                    event_hidden = transformer(event_hidden, self.src_mask)
                event_logits = self.event_logits_head(event_hidden)
                event_prob = F.softmax(event_logits[:, -1], dim=-1)
                show_infos["event_prob"] = event_prob[:, 1].detach().cpu().numpy().tolist()
        ...
        # KL regularization
        """
        1. calculate the distribution of event
        """
        event_hidden = z
        event_hidden = self.pos_encoder(event_hidden)
        for transformer in self.event_transformers:
        ...
        event_logits = self.event_logits_head(event_hidden)
        """
        2. calculate the kl loss of event variable
        """
        event_log_prob = F.log_softmax(event_logits, dim=-1)  # shape:b*t*2
        show_infos["event_occur_prob_dist"]= torch.exp(event_log_prob[:, :, 1]).detach().cpu().numpy()
        # construct inital prior
        event_prob_initial_prior = torch.zeros_like(event_log_prob[:, :1, :])
        event_prob_initial_prior[:, :, 0] = 1 - 1e-5
        event_prob_initial_prior[:, :, 1] = 1e-5
        event_log_prob_initial_prior = torch.log(event_prob_initial_prior)
        # construct prior
        event_log_prob_prior = torch.cat((event_log_prob_initial_prior, event_log_prob[:, :-1]), dim=-2)
        # event_prob_prior = torch.zeros_like(event_log_prob)
        # event_prob_prior[:, :, 0] = 1 - 1e-5
        # event_prob_prior[:, :, 1] = 1e-5
        # event_log_prob_prior = torch.log(event_prob_prior)
        event_log_prob_poster = event_log_prob
        # event_log_prob_prior, event_log_prob_poster = event_log_prob[:, :-1], event_log_prob[:, 1:]
        # event kl loss
        event_kl_loss = (torch.exp(event_log_prob_poster)*(event_log_prob_poster - event_log_prob_prior)).sum(dim=-1)
        show_infos["event_kl_loss_dist"]= event_kl_loss.detach().cpu().numpy()
        
        event_kl_loss = event_kl_loss.mean()
        aux_loss_infos["event_kl_loss"] = event_kl_loss
        show_infos["event_kl_loss"] = event_kl_loss.item()
        """
        calculate the kl loss of z, which is weighted by event_prob
        """
        prior_mu = torch.cat((torch.zeros_like(mu[:, :1]), mu[:, :-1]), dim=-2)
        poster_mu = mu
        prior_log_std = torch.cat((torch.zeros_like(log_std[:, :1]), log_std[:, :-1]), dim=-2)
        poster_log_std = log_std
        prior_var, poster_var = torch.exp(2*prior_log_std), torch.exp(2*poster_log_std)
        kl_loss = 0.5*((poster_var + torch.square(poster_mu-prior_mu))/(prior_var+1e-10)) - poster_log_std + prior_log_std
        kl_loss = kl_loss.sum(dim=-1)
        show_infos["self_kl_loss_dist"]=kl_loss.detach().cpu().numpy()

        prior_mu_event, prior_log_std_event = torch.zeros_like(mu), torch.zeros_like(log_std)
        prior_var_event = torch.exp(2*prior_log_std)
        kl_loss_event = 0.5*((poster_var + torch.square(poster_mu-prior_mu_event))/(prior_var_event+1e-10)) - poster_log_std + prior_log_std_event
        kl_loss_event = kl_loss_event.sum(dim=-1)
        show_infos["event_kl_loss_dist"]=kl_loss_event.detach().cpu().numpy()
        # weight by event_log_prob
        kl_loss = kl_loss * torch.exp(event_log_prob_poster)[:, :, 0] + kl_loss_event*torch.exp(event_log_prob_poster)[:, :, 1]
...
if __name__ == "__main__":
    ...
    hidden = model(x)
    hidden, aux_losses, mape = model(x, aux_loss=True, add_noise=True)
